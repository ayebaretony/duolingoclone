...
- **Import only the SDK module(s) a file actually needs.** Types and controllers (`ChatClient`, `UserInfo`, `Token`, `ChatChannelListController`, etc.) live in `StreamChat`; UIKit views and view controllers (`ChatChannelListVC`, `ChatMessageListVC`, `ComposerVC`, etc.) live in `StreamChatUI`. A file that only instantiates controllers needs only `import StreamChat`; a file that only lays out views needs only `import StreamChatUI`. Only import both in dedicated bridge files (e.g., a coordinator or `AppDelegate`) that wire controllers to view controllers — this is especially important when combining Chat + Video, where importing both SDKs in a single file causes "Ambiguous use of 'init'" for shared type names like `User` and `Token`. See [`COMBINED-CHAT-VIDEO.md`](COMBINED-CHAT-VIDEO.md) for the full type-name mapping table.
